    ...
    if (isset($movie['times']) && is_array($movie['times'])) {
        ...
        foreach ($grouped_times as $format => $times) {
            ...
            foreach ($times as $time) {
                $zawartosc .= "<a href='skrypty/rezerwacja.php?movie=" . urlencode($movie['title']) . "&time=" . urlencode($time) . "' class='showtime-button'>$time</a>";
            }
            $zawartosc .= "</div>";
        }
        $zawartosc .= "</div>";
...
?>
    ...
    <a href='skrypty/rezerwacja.php?movie=<?= urlencode($movie['title']) ?>&time=<?= urlencode($time) ?>' class='showtime-button'>
